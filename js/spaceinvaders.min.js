...
break}}d&&(this.invaders.splice(i--,1),t.sounds.playSound("bang"))}for(var g=this.boss,p=!1,f=0;f<this.rockets.length;f++){var o=this.rockets[f];if(o.x>=g.x-g.width/2&&o.x<=g.x+g.width/2&&o.y>=g.y-g.height/2&&o.y<=g.y+g.height/2){
this.rockets.splice(f--,1),p=!0,t.score+=this.config.pointsPerInvader;break}}p&&(this.boss.hp=this.boss.hp-20,0===this.boss.hp,t.sounds.playSound("bang"));for(var u={},i=0;i<this.invaders.length;i++){
var r=this.invaders[i];(!u[r.file]||u[r.file].rank<r.rank)&&(u[r.file]=r)}for(var i=0;i<this.config.invaderFiles;i++){var r=u[i];if(r){var y=this.bombRate*e;y>Math.random()&&this.bombs.push(new Bomb(r.x,r.y+r.height/2,this.bombMinVelocity+Math.random()*(this.bombMaxVelocity-this.bombMinVelocity)));
...
i.fillStyle="#ff5555";for(var s=0;s<this.bombs.length;s++){var n=this.bombs[s];i.fillRect(n.x-2,n.y-2,4,4)}i.fillStyle="#ff0000";for(var s=0;s<this.rockets.length;s++){
var a=this.rockets[s];i.fillRect(a.x,a.y-2,8,8),i.drawImage(gameImages.keycaps[0],a.x,a.y-2,8,8)}var r=t.gameBounds.bottom+(t.height-t.gameBounds.bottom)/2+7;i.font="14px Arial",
i.fillStyle="#ffffff";var l="Lives: "+t.lives;i.textAlign="left",i.fillText(l,t.gameBounds.left,r),l="Score: "+t.score+", Level: "+t.level,i.textAlign="right",i.fillText(l,t.gameBounds.right,r),
