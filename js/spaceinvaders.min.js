...
this.stage=i,this.hp=1e3,this.movedirection=-1,this.width=80,this.height=40}function GameState(t,e,i,s,o,h){this.updateProc=t,this.drawProc=e,this.keyDown=i,this.keyUp=s,
this.enter=o,this.leave=h}function Sounds(){this.audioContext=null,this.sounds={}}GameImages.prototype.keyboard=function(){var t=new Image;t.src="data:image/png;base64,"+this.keyboardImageCode,
this.keyboard=t};var gameImages=new GameImages;gameImages.keyboard(),Game.prototype.initialise=function(t){this.gameCanvas=t,this.width=t.width,this.height=t.height,
this.gameBounds={left:t.width/2-this.config.gameWidth/2,right:t.width/2+this.config.gameWidth/2,top:t.height/2-this.config.gameHeight/2,bottom:t.height/2+this.config.gameHeight/2
...
for(var i=this.config.invaderRanks,s=this.config.invaderFiles,o=[],h=0;h<i;h++)for(var n=0;n<s;n++)o.push(new Invader(t.width/2+200*(s/2-n)/s,t.gameBounds.top+20*h,h,n,"Invader"));
...
}var h=!1,n=!1,a=!1;for(i=0;i<this.invaders.length;i++){var r=this.invaders[i],l=r.x+this.invaderVelocity.x*e,d=r.y+this.invaderVelocity.y*e;h===!1&&l<t.gameBounds.left?h=!0:n===!1&&l>t.gameBounds.right?n=!0:a===!1&&d>t.gameBounds.bottom&&(a=!0),
h||n||a||(r.x=l,r.y=d)}for(this.invadersAreDropping&&(this.invaderCurrentDropDistance+=this.invaderVelocity.y*e,this.invaderCurrentDropDistance>=this.config.invaderDropDistance&&(this.invadersAreDropping=!1,
this.invaderVelocity=this.invaderNextVelocity,this.invaderCurrentDropDistance=0)),h&&(this.invaderCurrentVelocity+=this.config.invaderAcceleration,this.invaderVelocity={
x:0,y:this.invaderCurrentVelocity},this.invadersAreDropping=!0,this.invaderNextVelocity={x:this.invaderCurrentVelocity,y:0}),n&&(this.invaderCurrentVelocity+=this.config.invaderAcceleration,
...
this.boss.x<200&&(this.boss.movedirection=-1),this.boss.x=this.boss.x-e*this.boss.movedirection*30,a&&(this.lives=0),i=0;i<this.invaders.length;i++){for(var r=this.invaders[i],c=!1,f=0;f<this.rockets.length;f++){
var o=this.rockets[f];if(o.x>=r.x-r.width/2&&o.x<=r.x+r.width/2&&o.y>=r.y-r.height/2&&o.y<=r.y+r.height/2){this.rockets.splice(f--,1),c=!0,t.score+=this.config.pointsPerInvader;
break}}c&&(this.invaders.splice(i--,1),t.sounds.playSound("bang"))}for(var p=this.boss,u=!1,f=0;f<this.rockets.length;f++){var o=this.rockets[f];if(o.x>=p.x-p.width/2&&o.x<=p.x+p.width/2&&o.y>=p.y-p.height/2&&o.y<=p.y+p.height/2){
this.rockets.splice(f--,1),u=!0,t.score+=this.config.pointsPerInvader;break}}u&&(this.boss.hp=this.boss.hp-20,0===this.boss.hp,t.sounds.playSound("bang"));for(var g={},i=0;i<this.invaders.length;i++){
var r=this.invaders[i];(!g[r.file]||g[r.file].rank<r.rank)&&(g[r.file]=r)}for(var i=0;i<this.config.invaderFiles;i++){var r=g[i];if(r){var v=this.bombRate*e;v>Math.random()&&this.bombs.push(new Bomb(r.x,r.y+r.height/2,this.bombMinVelocity+Math.random()*(this.bombMaxVelocity-this.bombMinVelocity)));
...
i.drawImage(gameImages.keyboard,this.ship.x-this.ship.width/2,this.ship.y-this.ship.height/2,80,42),i.fillStyle="#006600";for(var s=0;s<this.invaders.length;s++){
var o=this.invaders[s];i.fillText("Test",o.x,o.y),i.fillRect(o.x-o.width/2,o.y-o.height/2,o.width,o.height)}i.fillStyle="#008800";var h=this.boss;i.fillRect(h.x-h.width/2,h.y-h.height/2,h.width,h.height),
console.log(h),i.fillStyle="#ff5555";for(var s=0;s<this.bombs.length;s++){var n=this.bombs[s];i.fillRect(n.x-2,n.y-2,4,4)}i.fillStyle="#ff0000";for(var s=0;s<this.rockets.length;s++){
var a=this.rockets[s];i.fillRect(a.x,a.y-2,1,4)}var r=t.gameBounds.bottom+(t.height-t.gameBounds.bottom)/2+7;i.font="14px Arial",i.fillStyle="#ffffff";var l="Lives: "+t.lives;
i.textAlign="left",i.fillText(l,t.gameBounds.left,r),l="Score: "+t.score+", Level: "+t.level,i.textAlign="right",i.fillText(l,t.gameBounds.right,r),this.config.debugMode&&(i.strokeStyle="#ff0000",
